...
\end{equation}
The upper integration limit can be chosen arbitrarily as long as it's outside the bubble, as outside the bubble $v=0$.
...
In these equations $\xi_\text{max} = \max (v_w, \xi_sh)$.
However, one can choose $\xi_\text{max}$ to be arbitrarily larger without affecting the results,
as all three quantities go to zero outside the fluid shell due to $v=0$, $w=w_n$ and $\theta = \theta_n$.
...
Kappa
\begin{equation}
\kappa \equiv \frac{e_K}{| \Delta e_\theta |}
\end{equation}
...
\end{equation}
\todo{Define wbar. In PTtools code it's untested and therefore unreliable.}
