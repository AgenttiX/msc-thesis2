...
Let us start from classical thermal physics.
The
...
In general relativity matter and energy content of space is described by the energy-momentum tensor $T^{\mu \nu}$, also known as the stress-energy tensor.}
...
\end{equation}
Inserting eq. \ref{eq:ep_tensor} results in the bubble wall junction conditions
...
&= \theta_s - \theta_b
\end{align}
With this we can define the transition strength as
\begin{equation}
...
To get expressions for $\tilde{v}_+$ and $\tilde{v}_-$ that are independent of $r$ we can extract $r$ from both equations and mark those as equal, resulting in the equation
\begin{equation}
...
\end{align}
Both of these are basic second-order solutions, but in the first the discriminant has been simplified.
