...
\end{align}
The spectral density of the time derivative of the perturbations $P_{\dot{h}}(\mathbb{k},t)$ is defined by
\cite[eq. 3.4]{hindmarsh_gw_pt_2019}
\begin{equation}
\langle \dot{h}_\mathbb{k}^{ij}(t) \dot{h}_{\mathbb{k}}^{ij}(t) \rangle = P_{\dot{h}}(\mathbb{k},t) (2\pi)^3 \delta (\mathbb{k} + \mathbb{k}').
\end{equation}

\begin{align}
\Lambda_{ij,kl}(\mathbb{k}) &= P_{ik}(\mathbb{k}) P_{jl}(\mathbb{k}) - \frac{1}{2} P_{ij}(\mathbb{k}) P_{kl}(\mathbb{k}) \\
P_{ij}(\mathbb{k}) &= \delta_{ij} - \hat{k}_i \hat{k}_j
\end{align}
...
\begin{align}
e_{gw}
&= \frac{1}{32 \pi G} \langle \dot{h}_{ij}^{TT} \dot{h}_{ij}^{TT} \rangle
